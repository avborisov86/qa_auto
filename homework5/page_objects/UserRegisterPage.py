...
class UserRegisterPage(BasePage):
    ...
    def fill_in_input(self, value: str):
        if value == 'First Name':
            self.logger.info(
                "Filling in input: {} with data {}".format(value, UserRegisterPageTestData.FIRST_NAME_DATA))
            ...
            self.logger.info(
                "Filling in input: {} with data {}".format(value, UserRegisterPageTestData.LAST_NAME_DATA))
            element = self.find_element(UserRegisterPage.LAST_NAME_INPUT)
            element.send_keys(UserRegisterPageTestData.LAST_NAME_DATA)
        elif value == "E-Mail":
            self.logger.info(
                "Filling in input: {} with data {}".format(value, UserRegisterPageTestData.EMAIL_DATA + str(
                    random.randint(1, 1000)) + "_" + random.choice(
            ...
            self.logger.info(
                "Filling in input: {} with data {}".format(value, UserRegisterPageTestData.TELEPHONE_DATA))
            element = self.find_element(UserRegisterPage.TELEPHONE_INPUT)
            element.send_keys(UserRegisterPageTestData.TELEPHONE_DATA)
        elif value == "Password":
            self.logger.info(
                "Filling in input: {} with data {}".format(value, UserRegisterPageTestData.PASS_DATA))
            ...
            self.logger.info(
                "Filling in input: {} with data {}".format(value, UserRegisterPageTestData.PASS_CONFIRM_DATA))
