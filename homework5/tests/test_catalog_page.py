...
Task 5: Catalog page https://demo.opencart.com/index.php?route=product/category&path=18 coverage by automated tests
Tests:
1. Проверка видимости элемента "боковое меню" на странице
2. Проверка видимости элемента "баннер" на странице
3. Проверка видимости элемента "кнопка вывода - list" на странице
4. Проверка видимости элемента "кнопка вывода - grid" на странице
5. Проверка видимости 5 продуктовых карточек на странице
...
def test_aside_menu_visibility(browser, cat_url):
    ...
    assert CatalogPageTestData.ASIDE_MENU_ID == catalog_page.get_property(aside_menu_elem,
...
def test_banner_visibility(browser, cat_url):
    ...
    assert CatalogPageTestData.BANNER_ID == catalog_page.get_property(banner_elem,
...
def test_list_btn_visibility(browser, cat_url):
    ...
    assert CatalogPageTestData.LIST_VIEW_BTN_ID == catalog_page.get_property(list_btn,
...
def test_grid_btn_visibility(browser, cat_url):
    ...
    assert CatalogPageTestData.GRID_VIEW_BTN_ID == catalog_page.get_property(grid_btn,
