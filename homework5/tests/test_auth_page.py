...
Task 5: Catalog page https://demo.opencart.com/admin/ coverage by automated tests
Tests:
1. Проверка видимости элемента "заголовок формы аутентификации"
2. Проверка видимости элемента "поле ввода логина"
3. Проверка видимости элемента "поле ввода пароля"
4. Проверка видимости элемента "ссылка Забыли пароль"
5. Проверка видимости элемента "кнопка Войти"
...
def test_form_title_visibility(browser, auth_url):
    ...
    assert AuthPageTestData.AUTH_FORM_TITLE_CHECK == auth_page.get_property(user_form_title,
...
def test_login_input_visibility(browser, auth_url):
    ...
    assert AuthPageTestData.LOGIN_INPUT_ID == auth_page.get_property(login_inp,
...
def test_password_input_visibility(browser, auth_url):
    ...
    assert AuthPageTestData.PASS_INPUT_ID == auth_page.get_property(pass_inp,
...
def test_forgot_password_visibility(browser, auth_url):
    ...
    assert AuthPageTestData.FORGOT_PASS_TEXT == auth_page.get_property(forgot_btn,
...
def test_login_btn_visibility(browser, auth_url):
    ...
    assert AuthPageTestData.LOGIN_BTN_TYPE == auth_page.get_property(login_btn,
                                                                     'type'), "Needs login button type should be equal to 'submit'!"
