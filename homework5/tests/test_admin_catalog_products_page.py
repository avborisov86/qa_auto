...
Task 5: Admin catalog products page http://127.0.0.1:8081/admin/index.php?route=catalog/product&user_token=bVRn8Sxc0qKlXaxcLPIvQm2zaRsFYo39 coverage by automated tests
Tests:
1. Проверка удаления случайного товара из каталога
2. Проверка удаления всех товаров из каталога
3. Проверка добавления в каталог нового товара
...
def test_add_product(browser, local_admin_url):
    ...
    assert admin_page.get_quantity(
        AdminCatalogProductsPage.PRODUCT_ROW) == total_products_qty + 1, "Needs total products quantity to be equal to: total + 1!"
