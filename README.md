...
2. Логика для парсинга данных из файла "access.log" и сохранения их в нужном формате расположена в файле "parser.py"
   ...
   _не требует передачи дополнительных параметров_
   
   get_requests_quantity() - считает количество выполненных запросов (общее количество, GET, POST, HEAD)
   _требует передачи дополнительных параметров: 
   1 параметр: строка с полным указанием пути до файла, в котором производим поиск
   2 параметр: 'all_requests' - поиск всех выполняемых запросов
   'get_requests' - поиск всех запросов типа GET
   'post_requests' - поиск всех запросов типа POST
   'head_requests' - поиск всех запросовтипа HEAD_
   
   find_most_freq_requests() - ищет топ 3 IP адресов, с которых было сделано наибольшее количество запросов
   _требует передачи дополнительных параметров:
   1 параметр: строка с полным указанием пути до файла, в котором производим поиск_
   
   find_slowest_requests() - ищет топ 3х самых долгих запросов
   _требует передачи дополнительных параметров:
   1 параметр: строка с полным указанием пути до файла, в котором производим поиск_
